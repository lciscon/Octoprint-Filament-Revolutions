...
class FilamentSensorsRevolutions(octoprint.plugin.StartupPlugin,
    ...
    def runout_sensor_callback(self, _):
        ...
        if ((self.runnout_tool > -1) and (self.current_tool() != self.runout_tool)):
    ...
    def jam_sensor_callback(self, _):
        ...
        # don't do anything if the active tool does not match the jam tool (if set)
        if ((self.jam_tool > -1) and (self.current_tool() != self.jam_tool)):
    ...
    def process_queued(self, comm_instance, phase, cmd, cmd_type, gcode, *args, **kwargs):
        self._logger.info("Processings2: %s" % cmd)
        if cmd.startswith("T"):
            self._active_tool = int(cmd[1:])
...
def __plugin_load__():
    ...
    __plugin_hooks__ = {
        ...
        "octoprint.comm.protocol.gcode.queued": __plugin_implementation__.process_queued,
    }
